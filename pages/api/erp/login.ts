...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    await pool.query(`CREATE TABLE IF NOT EXISTS erp_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) NOT NULL, email VARCHAR(255) UNIQUE NOT NULL, password VARCHAR(255) NOT NULL, role ENUM('admin','manager','employee') DEFAULT 'employee', department VARCHAR(100), salary DECIMAL(10,2) DEFAULT 0, joining_date DATE, active TINYINT(1) DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);
