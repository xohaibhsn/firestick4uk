...
export async function generateSubscriptionMetadata(): Promise<Metadata> {
  ...
  const canonical = resolveSubscriptionCanonical(
    route.slug,
...
export async function renderSubscriptionLandingPage() {
  const content = await getSubscriptionContent();
  const route = await getSubscriptionSlugConfig();
  const pageUrl = route.pageUrl || subscriptionPageUrl(DEFAULT_SUBSCRIPTION_SLUG);
  ...
  const pageTitle = cms(
    ...
    "IPTV Subscriptions UK — Flexible Plans for Firestick & Smart Devices"
  );
    ...
    <>
      <BreadcrumbSchema
        items={[
          ...
          { name: "IPTV Subscriptions UK", url: pageUrl },
        ]}
      />
      <script
