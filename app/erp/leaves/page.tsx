...
function LeavesContent({ user }: { user: any }) {
  ...
  const load = () => {
    const url = user.role==="admin" ? "/api/erp/leaves" : `/api/erp/leaves?employee_id=${user.id}`;
  ...
  return (
    <div>
      ...
      <div className="erp-card">
        <div className="erp-section-header"><div className="erp-section-title">Leave Requests</div></div>
        <div className="erp-table-wrap">
          <table>
            <thead><tr><th>Date Filed</th>{user.role==="admin"&&<th>Employee</th>}<th>Type</th><th>From</th><th>To</th><th>Days</th><th>Reason</th><th>Status</th>{user.role==="admin"&&<th>Actions</th>}</tr></thead>
            <tbody>
              ...
              {leaves.map((l:any)=>(
                <tr key={l.id}>
                  ...
                  {user.role==="admin"&&<td style={{fontWeight:600}}>{l.employee_name}</td>}
                  ...
                  {user.role==="admin"&&<td>{l.status==="pending"&&<div style={{display:"flex",gap:6}}><button className="erp-btn erp-btn-green erp-btn-sm" onClick={()=>decide(l.id,"approved")}>Approve</button><button className="erp-btn erp-btn-red erp-btn-sm" onClick={()=>decide(l.id,"rejected")}>Reject</button></div>}</td>}
                </tr>
              ))}
            </tbody>
          </table>
