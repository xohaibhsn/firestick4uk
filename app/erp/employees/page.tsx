...
function EmpContent({ user }: { user: any }) {
  ...
  const [form, setForm] = useState({ name:"", email:"", password:"", role:"employee", department:"", salary:"", joining_date:"" });
  const [msg, setMsg] = useState("");

  const load = () => { fetch("/api/erp/employees").then(r=>r.json()).then(d=>setEmployees(Array.isArray(d)?d:[])).catch(()=>{}); };
  useEffect(()=>{ load(); },[]);

  const openNew = () => { setForm({name:"",email:"",password:"",role:"employee",department:"",salary:"",joining_date:""}); setModal("new"); setMsg(""); };
  const openEdit = (e:any) => { setForm({name:e.name,email:e.email,password:"",role:e.role,department:e.department||"",salary:e.salary||"",joining_date:e.joining_date||""}); setModal(e); setMsg(""); };
    ...
    <div>
      ...
      <div className="erp-card">
        <div className="erp-table-wrap">
          <table>
            <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Role</th><th>Department</th><th>Salary</th><th>Joined</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {employees.length===0 && <tr><td colSpan={9} style={{textAlign:"center",color:"rgba(255,255,255,0.25)",padding:24}}>No employees found</td></tr>}
              {employees.map((e:any)=>(
                <tr key={e.id}>
                  ...
                  <td><button className="erp-btn erp-btn-outline erp-btn-sm" style={{marginRight:6}} onClick={()=>openEdit(e)}>Edit</button>{e.id!==user.id&&e.active&&<button className="erp-btn erp-btn-red erp-btn-sm" onClick={()=>deactivate(e.id)}>Deactivate</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="erp-modal-overlay" onClick={()=>setModal(null)}>
          <div className="erp-modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="erp-grid-2">
              ...
              <div className="erp-field"><label>Role</label><select className="erp-select" value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))}><option value="employee">Employee</option><option value="manager">Manager</option><option value="admin">Admin</option></select></div>
            </div>
            <div className="erp-grid-2">
